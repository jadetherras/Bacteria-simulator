...
Vec2d getHelpPosition()
{
    return { getShortConfig().window_simulation_width,
             getShortConfig().window_simulation_height*1/4 };
}
/*!
    ...
    ,isStatsOn(false)
{
    ...
    mLabBackground.setOutlineColor(sf::Color::White);
...
void Application::run()
{
    ...
    controlBackground.setFillColor(sf::Color(32, 64, 100));

    // Create the control background (grey board)
    auto helpBackground = sf::RectangleShape();
    helpBackground.setSize(getHelpSize());
    helpBackground.setFillColor(sf::Color(32, 64, 100)); //168
...
void Application::drawControls(sf::RenderWindow& target)
{
    ...
    drawTitle(target, sf::Color(255, 165, 0), LEGEND_MARGIN, lastLegendY, FONT_SIZE);
    lastLegendY += FONT_SIZE + 4;
    drawNames(target, sf::Color(255, 165, 0), LEGEND_MARGIN +45, lastLegendY, FONT_SIZE);
    lastLegendY += FONT_SIZE + 4;
...
{
    std::string text("BACTERIA SIMULATOR SV2020");
...
void Application::drawNames(sf::RenderWindow& target
                            , sf::Color color
                            , size_t xcoord
                            , size_t ycoord
                            , size_t font_size
...
{
    std::string text("Therras & Reibel");
    auto legend = sf::Text(text, getAppFont(), font_size);
    legend.setPosition(xcoord, ycoord);
#if SFML_VERSION_MAJOR >= 2 && SFML_VERSION_MINOR >= 4
    legend.setFillColor(color);
                                ...
                                )
{
    ...
    case STATS :
        ...
        break;
    case PROB :
        text = "Probabilite nutriment : ";
        text += to_nice_string(mLab->getProb_nutriment());
        break;
    default:
        /* nothing to do */
        break;
